...
namespace Best.MQTT
{
    ...
    public static class SessionHelper
    {
        ...
        /// </summary>
        internal static string GetRootSessionsDirectory(string host) => Path.Combine(new string[] { GetRootSaveFolder(), "MQTT", "Sessions", ToFileName(host) });
        ...
        private static SessionDef Create(string clientId)
        {
            ...
            sessionDef.ClientId = string.IsNullOrEmpty(clientId) ? ("Best.MQTT_" + Guid.NewGuid().ToString().Replace("-", string.Empty)) : clientId;
        ...
        private static SessionStoreDef LoadStore(string host)
        {
            ...
            {
                using (var jsonFile = new StreamReader(IOService.CreateFileStream(sessionStorePath, Best.HTTP.Shared.PlatformSupport.FileSystem.FileStreamModes.OpenRead)))
                    return Best.HTTP.JSON.LitJson.JsonMapper.ToObject<SessionStoreDef>(jsonFile);
            }
            else
            {
        ...
        {
            using (var jsonFile = new StreamWriter(IOService.CreateFileStream(GetSessionStoreJSonPath(host), Best.HTTP.Shared.PlatformSupport.FileSystem.FileStreamModes.Create)))
                Best.HTTP.JSON.LitJson.JsonMapper.ToJson(sessionStoreDef, new Best.HTTP.JSON.LitJson.JsonWriter(jsonFile));
        }
    }
    ...
    public sealed class Session
    {
        ...
        internal void Close()
        {
            this.PublishReceivedPacketIDs?.Dispose();
            this.ClientTopicAliasMapping?.Dispose();
            this.UnacknowledgedPackets?.Dispose();
            this.PublishReleasedPacketIDs?.Dispose();
            this.QueuedPackets?.Dispose();
        }
