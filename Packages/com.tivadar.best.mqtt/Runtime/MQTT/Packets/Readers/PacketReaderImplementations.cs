...
namespace Best.MQTT.Packets.Readers
{
    ...
    internal static class PacketReaderImplementations
    {
        ...
        public static List<ApplicationMessage> CreateApplicationMessages(MQTTClient client, Packet packet)
        {
            applicationMessages.Clear();

            if (packet.VariableHeaderFields.Properties.IsPresent(PacketProperties.SubscriptionIdentifier))
            {
                packet.VariableHeaderFields.Properties.ForEach(PacketProperties.SubscriptionIdentifier, sIdData =>
                    applicationMessages.Add(new ApplicationMessage(sIdData.Integer, packet)));
            }
            else
            {
                ...
                {
                    foreach (var subscription in client.FindSubscriptionsWithMatchingTopic(topicName))
                        applicationMessages.Add(new ApplicationMessage(subscription.ID, packet));
                }
            }
