...
namespace Best.MQTT.Packets.Readers
{
    internal static class IncomingPacketReader
    {
        internal static (bool success, Packet packet) TryToReadFrom(Transport transport)
        {
            ...
            if (stream.Length < 2)
            {
                HTTPManager.Logger.Information(nameof(IncomingPacketReader), $"{nameof(TryToReadFrom)}: Not enough data for fixed header! Available: {stream.Length:N0}", transport.Context);
            ...
            try
            {
                ...
                if (!success)
                {
                    HTTPManager.Logger.Information(nameof(IncomingPacketReader), $"{nameof(TryToReadFrom)}: Not enough data for remaining length! Available: {stream.Length:N0}", transport.Context);
                    return (false, Packet.Empty());
                }
                ...
                {
                    HTTPManager.Logger.Information(nameof(IncomingPacketReader), $"{nameof(TryToReadFrom)}: Not all data available for the whole packet! Available: {stream.Length:N0}, packet size: {packetSize:N0}", transport.Context);
                    return (false, Packet.Empty());
                }
            ...
            {
                HTTPManager.Logger.Exception(nameof(IncomingPacketReader), $"{nameof(TryToReadFrom)}", ex, transport.Context);
                return (false, Packet.Empty());
            }
        }
    }
