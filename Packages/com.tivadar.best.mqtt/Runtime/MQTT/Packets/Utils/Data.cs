...
namespace Best.MQTT.Packets.Utils
{
    ...
    public struct Data
    {
        ...
        public static Properties ReadProperties(Stream stream, UInt32 length, ref UInt32 remainingLength)
        {
            ...
            while (length > 0)
            {
                //Best.HTTPManager.Logger.Information(nameof(Data), $"{nameof(ReadProperties)}({stream.Position}/{stream.Length}, {length}, {remainingLength})");
                ...
                //Best.HTTPManager.Logger.Information(nameof(Data), $"{nameof(ReadProperties)} {remainingLengthBefore} - {remainingLength} = {remainingLengthBefore - remainingLength} => {length} - {remainingLengthBefore - remainingLength} => {length - remainingLengthBefore - remainingLength}");
